 ...
 */
void t_cocoa_generator::generate_cocoa_struct_is_equal_method(ofstream& out, t_struct* tstruct, t_service* service) {
...
void t_cocoa_generator::generate_cocoa_service_helpers(t_service* tservice) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    generate_cocoa_struct_interface(f_impl_, ts, false, tservice);
    generate_cocoa_struct_implementation(f_impl_, ts, false, false, tservice);
 ...
 */
void t_cocoa_generator::generate_function_helpers(t_function* tfunction, t_service* service) {
  if (tfunction->is_oneway()) {
    return;
  }
  ...
  generate_cocoa_struct_interface(f_impl_, &result, false, service);
  generate_cocoa_struct_implementation(f_impl_, &result, false, true, service);
}
...
// knock async code
void t_cocoa_generator::generate_cocoa_service_knock_async_protocol(ofstream& out, t_service* tservice) {
...
void t_cocoa_generator::generate_cocoa_service_knock_async_client_interface(ofstream& out,
                                                                      t_service* tservice) {
...
void t_cocoa_generator::generate_knock_send_function_body(ofstream& out,
                                                     t_service* tservice,
                                                     vector<t_function*>::const_iterator f_iter) {
   // TODO(mcslee): Message validation here, was the seqid etc ok?
...
void t_cocoa_generator::generate_cocoa_service_knock_async_client_implementation(ofstream& out,
                                                                           t_service* tservice) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (!(*f_iter)->is_oneway()) {
      ...
      if ((*f_iter)->get_returntype()->is_void()) {
        ...
        out << indent() << type_name((*f_iter)->get_returntype()) << " result = [self recv_"
...
string t_cocoa_generator::function_knock_async_signature(t_function* tfunction) {
  std::string result = "(void) " + tfunction->get_name() + argument_list(tfunction->get_arglist());
  ...
  if (tfunction->get_returntype()->is_void()) {
    ...
    result += "(void (^)(" + type_name(tfunction->get_returntype()) + " result)) success";
  }
