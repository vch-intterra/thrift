...
Thrift.TWebSocketTransport.prototype = {
    ...
    fnameToArgs: function (fname) {
        return new window[fname.split(":")[0] + "_" + fname.split(":")[1] + "_args"]();
    },

    fnameToResult: function (fname, result) {
        return new window[fname.split(":")[0] + "_" + fname.split(":")[1] + "_result"](result);
    },
...
Thrift.AsyncManager = (function () {
    var managerInstance;

    var createAsyncManager = function () {
        var seqId = 0;
        var pending = [];
        var clientCallbacks = []

        var put = function (seqId, callback) {
            pending[seqId] = callback;
        };

        var pop = function (seqId) {
            var result = pending[seqId];
            delete pending[seqId];
            return result;
        };

        var getNextSeqId = function () {
            return ++seqId;
        };

        var addClientCallback = function (methodName, callback) {
            clientCallbacks[methodName] = callback;
        }

        var getClientCallback = function (methodName) {
            return clientCallbacks[methodName];
        }

        return {
            put: put,
            pop: pop,
            getNextSeqId: getNextSeqId,
    ...
    return {
        getInstance: function () {
            if (!managerInstance) {
                managerInstance = createAsyncManager();
            }
...
Thrift.TMemoryOutputTransport = function () {
    this.buffer = "";
};

Thrift.TMemoryOutputTransport.prototype = {
...
Thrift.TMemoryInputTransport = function (data) {
    this.buffer = data;
};

Thrift.TMemoryInputTransport.prototype = {
